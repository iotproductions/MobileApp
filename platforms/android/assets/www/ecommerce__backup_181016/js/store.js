app.controller("storectrl", function ($scope, $rootScope) {
    ...
    var username=GetParamsPage().UserName;
    $scope.Data;
    $scope.DataPro;
    $scope.Back = function () {
        PopPage();
    ...
    GetDetailSeller(CustomerCodeShop,username, function (rel) {
        if (rel.success) {
            $scope.Data = rel.data;
...
function GetDetailSeller(cuscode,username, callback) {
    $.ajax({
        url: ipEcommerce + "/SellerChannel/GetInfoChannel", type: "GET", dataType: "JSON", data: { CustomerCode: cuscode,UserName:username },
        timeout: 4500,
